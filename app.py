...
CACHE_DIR = "/app/tile_cache"  # Путь для Render
...
def reproject_tile(tile_data, src_crs="EPSG:3395", dst_crs="EPSG:3857"):
    try:
            ...
            with memfile.open() as src:
                ...
                with rasterio.MemoryFile() as mem_dst:
                    with mem_dst.open(**kwargs) as dst:
                        ...
                        return dst.read(), dst.transform
    except Exception as e:
        logging.error(f"Reprojection error: {e}")
        return None, None

# Функция для сшивания двух тайлов (упрощенная)
...
async def get_tile(z: int, x: int, y: int):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if tile1_data and tile2_data:
            # Перепроецирование
            tile1_reprojected, _ = reproject_tile(tile1_data)
            tile2_reprojected, _ = reproject_tile(tile2_data)
